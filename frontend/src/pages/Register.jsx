...
const Register = () => {
  ...
  const passwordStrength = (pwd) => {
    if (!pwd) return null
    if (pwd.length < 6) return { label: 'Weak', color: 'bg-rose-400', width: 'w-1/4' }
    if (pwd.length < 10 || !/[0-9]/.test(pwd)) return { label: 'Fair', color: 'bg-amber-400', width: 'w-2/4' }
    if (!/[^a-zA-Z0-9]/.test(pwd)) return { label: 'Good', color: 'bg-yellow-400', width: 'w-3/4' }
              ...
              {strength && (
                <div className="pt-1 space-y-1">
                  ...
                  <p className="text-xs text-stone-400">
                    Strength: <span className="font-semibold text-stone-500">{strength.label}</span>
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full pl-10 pr-11 py-3 rounded-xl border text-stone-800 text-sm placeholder:text-stone-300 focus:outline-none focus:ring-2 focus:border-transparent transition duration-200 bg-stone-50/60
                ...
                {/* Match indicator */}
                {confirmPassword && (
                  <span className="absolute -translate-y-1/2 right-10 top-1/2">
